    ...
    public class CustomerController : ControllerBase
    {
        ...
        {         
            ...
            var response = furnitures.Select(f => new
            {
                ...
                Image = f.FurnitureSpecifications.Count > 0 && f.FurnitureSpecifications.FirstOrDefault().Attachments.Count > 0 ? f.FurnitureSpecifications.FirstOrDefault().Attachments.FirstOrDefault().Path : String.Empty,
        ...
        public async Task<IActionResult> Order(OrderViewModel model)
        {
            ...
            {
                return StatusCode(StatusCodes.Status500InternalServerError,
              new Response() { Status = "Error", Message = "An error occurs when adding new order"});
            }
            ...
            await _dbContext.SaveChangesAsync();
            if (newOrder.PaymentId != 1 || isCustomizeFurnitureOrder) return Ok(UrlPayment(newOrder.PaymentId, newOrder.OrderId));
